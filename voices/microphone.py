...
class Audio(object):
    """Streams raw audio from microphone. Data is received in a separate
    thread, and stored in a buffer, to be read from. """
    ...
    def __init__(self, callback=None, device=None,
                 input_rate=RATE_PROCESS, file=None):
        resampler = Resampler()
        
        def proxy_callback(in_data, frame_count, time_info, status):
            resampled_data = in_data
            if self.chunk is not None:
                in_data = self.wf.readframes(self.chunk)
                resampled_data = resampler.process(in_data, RATE_PROCESS/input_rate)
    ...
    def read_resampled(self):
        """Return a block of audio data resampled to 16kHz, blocking if
        necessary. """
...
class VADAudio(Audio):
    ...
    def vad_collector(self, padding_ms=300, ratio=0.75, frames=None):
        ...
        Determines voice activity by ratio of frames in padding_ms. Uses a
        buffer to include padding_ms prior to being triggered. Example:
...
def main():
    ...
    # model_file = 'data/output_graph.pb'
    # scorer_file = 'data/kenlm.scorer'

    model_file = 'data/deepspeech-0.8.2-models.tflite'
    scorer_file = 'data/deepspeech-0.8.2-models.scorer'
    ...
    vad_audio = VADAudio(aggressiveness=3, device=0, input_rate=MIC_RATE)
    ...
    for frame in frames:
        if frame is not None:
            ...
            try:
                ...
                requests.get(address, params=payload)
